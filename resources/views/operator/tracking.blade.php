...
<div class="max-w-7xl mx-auto p-8">
    ...
    <div class="bg-white rounded-[20px] shadow-sm w-full border border-gray-100 overflow-hidden" x-data="{ activeTab: 'all' }">
        <!-- Header -->
        ...
        @if($order->payment_status !== 'PAID')
            <div class="px-8 py-4 bg-amber-50 border-b border-amber-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-3 text-amber-700">
                    <i class="fa-solid fa-file-invoice-dollar text-xl"></i>
                    <div>
                        <h4 class="text-sm font-extrabold">Pesanan Belum Lunas</h4>
                        <p class="text-xs font-medium opacity-80">Pesanan masuk proses produksi namun pembayaran belum lunas. Anda dapat langsung mengonfirmasi jika customer membayar sekarang.</p>
                    </div>
                </div>
                <form action="{{ route('operator.tracking.confirmPayment', $order->id) }}" method="POST">
                    @csrf
                    <button type="submit" onclick="return confirm('Yakin ingin konfirmasi lunas pesanan ini?')" class="bg-brand-blue text-white hover:bg-indigo-700 transition px-5 py-2.5 rounded-xl text-xs font-bold shadow-sm flex items-center justify-center gap-2 whitespace-nowrap">
        ...
        <div class="px-8 py-5 bg-indigo-50/40 border-b border-indigo-100/60">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                @foreach($allSteps as $idx => $st)
                    ...
                    @endphp
                    <div @click="activeTab = (activeTab === {{ $st->id }}) ? 'all' : {{ $st->id }}; if(activeTab !== 'all') $refs.stepSelect.value = {{ $st->id }}" 
                         :class="{ 'ring-2 ring-brand-blue ring-offset-2': activeTab === {{ $st->id }} }" 
                         class="cursor-pointer p-2.5 rounded-xl border text-center text-xs font-bold transition flex flex-col justify-between items-center gap-1 {{ $isCompleted ? 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100' : ($isCurrentNext ? 'bg-brand-blue text-white border-brand-blue shadow-sm hover:bg-indigo-700' : 'bg-white border-gray-200 text-gray-400 hover:bg-gray-50') }}">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-4">
                    @forelse($order->production->logs ?? [] as $log)
                        <div x-show="activeTab === 'all' || activeTab === {{ $log->production_step_id }}" x-transition.opacity class="p-4 border border-gray-200/80 rounded-2xl bg-gray-50/50 space-y-2.5">
                ...
                @else
                    <div class="bg-white border border-indigo-100 rounded-2xl p-4 space-y-2 shadow-xs">
                        <div class="flex items-center justify-between text-xs">
                            <span class="font-extrabold text-gray-400 uppercase tracking-wider">Update / Edit Tahap Produksi</span>
                            <span class="bg-indigo-50 text-brand-blue px-3 py-1 rounded-full font-extrabold text-xs">Custom</span>
                        </div>
                        <h4 class="text-sm font-extrabold text-gray-500">
                            Pilih tahap yang ingin diupdate atau diedit ulang.
                        </h4>
                    </div>
                    ...
                    <form action="{{ route('operator.tracking.store', $order->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-2">Pilih Tahapan Produksi <span class="text-red-500">*</span></label>
                            <select x-ref="stepSelect" @change="activeTab = parseInt($event.target.value)" name="production_step_id" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm font-bold text-gray-700 outline-none focus:border-brand-blue bg-white" required>
                                @foreach($allSteps as $st)
                                    <option value="{{ $st->id }}" {{ ($nextStep && $nextStep->id == $st->id) ? 'selected' : '' }}>
                                        {{ $st->name }} {{ in_array($st->id, $completedStepIds) ? '(Selesai)' : '' }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-2">Catatan/Update <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium text-gray-700 outline-none focus:border-brand-blue bg-white" required>
                                ...
                                <option value="completed">Tahap Selesai (Selesai/Revisi Selesai)</option>
                            </select>
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">Upload Multi-Foto Bukti Pengerjaan</label>
                            <p class="text-[10px] text-gray-400 font-medium mb-2">Pilih foto bukti hasil pengerjaan (PNG, JPG, Max 10MB per foto)</p>
